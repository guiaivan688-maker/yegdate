...
export default function BuffetsPage() {
  ...
  const availableNeighbourhoods = useMemo(() => {
    ...
    data.forEach((cat) => cat.restaurants.forEach((r) => set.add(getNeighbourhood(r.address))));
    return NEIGHBOURHOODS.filter((n) => set.has(n));
  }, [data]);

  const filteredData = neighbourhood === "all"
    ? data
    : data
        .map((cat) => ({ ...cat, restaurants: cat.restaurants.filter((r) => getNeighbourhood(r.address) === neighbourhood) }))
        .filter((cat) => cat.restaurants.length > 0);
    ...
    <>
      ...
      {availableNeighbourhoods.length > 1 && (
        <section className="sticky top-16 z-30 bg-cream/95 backdrop-blur border-b border-black/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center gap-1.5">
        ...
        <div className="max-w-7xl mx-auto space-y-16">
          {filteredData.length === 0 && (
            <p className="text-center text-navy/60 py-12">
              {fr ? "Aucun buffet dans ce quartier." : "No buffets in this neighbourhood."}
            </p>
          )}
